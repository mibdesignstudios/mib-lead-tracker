...
const LIST_SQL = `SELECT l.*, a.code AS agent_code, a.name AS agent_name,
    (SELECT note FROM lead_updates u WHERE u.lead_id=l.id AND u.note IS NOT NULL ORDER BY u.created_at DESC LIMIT 1) AS last_note
...
export default route({
  GET: async (req, res) => {
    ...
    if (p.id) {
      ...
      const updates = await q("SELECT * FROM lead_updates WHERE lead_id=$1 ORDER BY created_at DESC", [lead.id]);
    ...
    const due = await q("SELECT COUNT(*)::int AS n FROM leads WHERE status='follow_up' AND follow_up_date <= CURRENT_DATE");
  ...
  POST: async (req, res) => {
    ...
    const dup = b.client_phone ? (await q("SELECT id FROM leads WHERE right(regexp_replace(coalesce(client_phone,''),'\\D','','g'),10)=$1 ORDER BY id LIMIT 1", [phone10(b.client_phone)]))[0] : null;
    const rows = await q(`INSERT INTO leads (agent_id, agent_name_raw, agent_phone_raw, client_name, client_phone, property_type, location, budget, timeline, service, notes, source, duplicate_of)
    ...
    await q("INSERT INTO lead_updates (lead_id, status, note, shared) VALUES ($1,'new',$2,true)", [rows[0].id, "Lead registered"]);
  ...
  PATCH: async (req, res) => {
    ...
    const cur = (await q("SELECT * FROM leads WHERE id=$1", [id]))[0];
    ...
    await q(`UPDATE leads SET agent_id=$1, client_name=$2, client_phone=$3, property_type=$4, location=$5, budget=$6, timeline=$7, service=$8,
    ...
    if (shared && (auto || note)) {
      await q("INSERT INTO lead_updates (lead_id, status, note, shared) VALUES ($1,$2,$3,true)", [id, statusChanged ? status : null, [auto, note].filter(Boolean).join(" — ")]);
    } else {
      // Stage changes are always visible to the partner; an internal note is stored separately.
      if (auto) await q("INSERT INTO lead_updates (lead_id, status, note, shared) VALUES ($1,$2,$3,true)", [id, statusChanged ? status : null, auto]);
      if (note) await q("INSERT INTO lead_updates (lead_id, status, note, shared) VALUES ($1,NULL,$2,false)", [id, note]);
    }
