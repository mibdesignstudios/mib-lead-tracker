...
export default route({
  GET: async (req, res) => {
    ...
    const rows = await q(`SELECT p.*, a.name AS agent_name, a.code AS agent_code, l.client_name
      FROM payments p JOIN agents a ON a.id=p.agent_id LEFT JOIN leads l ON l.id=p.lead_id
  ...
  POST: async (req, res) => {
    ...
    await q("INSERT INTO payments (agent_id, lead_id, amount, paid_on, note) VALUES ($1,$2,$3,COALESCE($4::date,CURRENT_DATE),$5)",
      [Number(b.agent_id), b.lead_id ? Number(b.lead_id) : null, amount, date, clean(b.note, 300)]);
    send(res, 201, { ok: true });
  },
  DELETE: async (req, res) => {
    await requireAdmin(req);
    await q("DELETE FROM payments WHERE id=$1", [Number(getQuery(req).id)]);
